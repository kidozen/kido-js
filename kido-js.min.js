...
a.user).replace("[Password]",a.pass).replace("[ApplyTo]",a.scope),headers:{"Content-Type":"application/soap+xml; charset=utf-8"}})}function b(a){return $.ajax({url:a.endpoint,data:{wrap_name:a.user,wrap_password:a.pass,wrap_scope:a.scope},type:"POST",dataType:"text"})}var g=this;if(!(this instanceof Kido))return new Kido;this.name=c||"local";this.marketplace=d;this.local="local"===this.name;this.authenticated=(this.active=!!d)?!1:!0;this.active&&(this.authConfig=$.ajax({url:this.marketplace+"/publicapi/apps?name="+
this.name}).pipe(function(a){g.url=a[0].url.substring(0,a[0].url.length-1);return a[0].authConfig}));this.authenticate=function(h,e,c){if(!this.active)throw Error("No need to authenticate to this Web App");return this.token=this.authConfig.pipe(function(d){var f=d.identityProviders;c=c||Object.keys(f)[0];f=f[c];if(!f)throw Error("Invalid Identity Provider");if(!f.protocol)throw Error("Invalid Identity Provider Protocol");var l;if("wrapv0.9"===f.protocol.toLowerCase())l=b;else if("ws-trust"===f.protocol.toLowerCase())l=
a;else throw Error("Protocol not supported yet.");return l({user:h,pass:e,scope:d.authServiceScope,endpoint:f.endpoint}).pipe(function(a){a=(/<Assertion(.*)<\/Assertion>/.exec(a)||[])[0];if(!a)throw Error("Unable to get a token from IDP");return $.ajax({url:d.authServiceEndpoint,type:"POST",data:{wrap_assertion:a,wrap_scope:d.applicationScope,wrap_assertion_format:"SAML"}}).pipe(function(a){g.authenticated=!0;a.token='WRAP access_token="'+a.rawToken+'"';return a})})})};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",
error:"Invalid URL"});var b=$.extend({},{type:"POST",cache:!0},a);b.data&&!b.contentType&&(b.contentType="application/json");return g.active&&g.authenticated?g.token.pipe(function(a){b.url=g.url+b.url;b.headers=b.headers||{};b.headers.authorization=a.token;return $.ajax(b)}):$.ajax(b)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return g.send(b)};this.post=function(a,b,c){c=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},c||{});return g.send(c)};this.del=function(a,b){b=
$.extend({url:a,type:"DELETE"},b||{});return g.send(b)}};var KidoConfig=function(c){var d=this;if(!(this instanceof KidoConfig))return new KidoConfig(c);if(!c)throw"The 'kidoApp' argument is required by the KidoConfig class.";this.app=c;this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return d.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return d.app.get("/config/"+a)};this.getAll=function(){return d.app.get("/config")};this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";
return d.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(c){var d=this;if(!(this instanceof KidoEmail))return new KidoEmail(c);if(!c)throw"The 'kidoApp' argument is required by the KidoEmail class.";this.app=c;this.send=function(a,b,g,c,e,j,k){if(a&&"object"===typeof a){var f=a;a=f.from;b=f.to;g=f.subject;c=f.bodyText;e=f.bodyHtml;j=f.attachments;k=f.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!j&&e instanceof Array&&(j=e,e=null);f={to:b,from:a};
"string"===typeof g&&0<g.length&&(f.subject=g);"string"===typeof c&&0<c.length&&(f.bodyText=c);"string"===typeof e&&0<e.length&&(f.bodyHtml=e);j instanceof Array&&(f.attachments=j);return d.app.send({url:"/email",type:"POST",data:JSON.stringify(f),timeout:k||9E5})};this.attach=function(a,b,g){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=a.files[0]}if(a instanceof File){var c=
...
return d.objectSet.invoke({indexes:"indexes/"+a,settings:{type:"DELETE"}})};this.create=function(a,b,c,h,e,j,k,f){if(!a)throw"The 'spec' argument is required to create an index.";b||(b=!1);c||(c=!1);h||(h=!1);e||(e=!1);j||(j=!1);a={spec:a,options:{safe:b,unique:c,sparse:h,background:e,dropDups:j}};k&&"0"!=k.toString()&&(a.options.min=k);f&&"0"!=f.toString()&&(a.options.max=f);k={indexes:"indexes",settings:{type:"POST",data:JSON.stringify(a)}};return d.objectSet.invoke(k)}};
