...
a.user).replace("[Password]",a.pass).replace("[ApplyTo]",a.scope),headers:{"Content-Type":"application/soap+xml; charset=utf-8"}})}function b(a){return $.ajax({url:a.endpoint,data:{wrap_name:a.user,wrap_password:a.pass,wrap_scope:a.scope},type:"POST",dataType:"text"})}if(!(this instanceof Kido))return new Kido;var g=this,f=null,e=null,k=null;this.name=c||"local";this.marketplace=d;this.local="local"===this.name;this.authenticated=(this.active=!!d)?!1:!0;this.active&&(this.authConfig=$.ajax({url:this.marketplace+
"/publicapi/apps?name="+this.name}).pipe(function(a){if(!a||!a.length)return $.Deferred().reject("Application configuration not found.");g.url=a[0].url.substring(0,a[0].url.length-1);return a[0].authConfig}));this.authenticate=function(c,d,p){return this.active?this.token=this.authConfig.pipe(function(n){var m=n.identityProviders;p=p||Object.keys(m)[0];m=m[p];if(!m)return $.Deferred().reject("Invalid Identity Provider");if(!m.protocol)return $.Deferred().reject("Invalid Identity Provider Protocol");
var q;if("wrapv0.9"===m.protocol.toLowerCase())q=b;else if("ws-trust"===m.protocol.toLowerCase())q=a;else return $.Deferred().reject("Protocol not supported yet.");return q({user:c,pass:d,scope:n.authServiceScope,endpoint:m.endpoint}).pipe(function(a){return(a=(/<Assertion(.*)<\/Assertion>/.exec(a)||[])[0])?$.ajax({url:n.authServiceEndpoint,type:"POST",data:{wrap_assertion:a,wrap_scope:n.applicationScope,wrap_assertion_format:"SAML"}}).pipe(function(a){if(!a||!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");
g.authenticated=!0;f=c;e=d;k=p;a.token='WRAP access_token="'+a.rawToken+'"';var b=decodeURIComponent(a.rawToken).split("&");a.claims=b;for(var m in b){var n=b[m];if(-1<n.indexOf("ExpiresOn")){a.expiresOn=1E3*~~n.split("=")[1]-2E4;break}}return a}):$.Deferred().reject("Unable to get a token from IDP")})}):$.Deferred().reject("No need to authenticate to this Web App")};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var b=$.extend({},{type:"POST",
cache:!0},a);b.data&&!b.contentType&&(b.contentType="application/json");return g.active&&g.authenticated?g.token.pipe(function(a){return a.expiresOn<(new Date).getTime()?g.authenticate(f,e,k):a}).pipe(function(a){b.url=g.url+b.url;b.headers=b.headers||{};b.headers.authorization=a.token;return $.ajax(b)}):$.ajax(b)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return g.send(b)};this.post=function(a,b,f){f=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},f||{});return g.send(f)};
this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return g.send(b)}};var KidoConfig=function(c){var d=this;if(!(this instanceof KidoConfig))return new KidoConfig(c);if(!c)throw"The 'kidoApp' argument is required by the KidoConfig class.";this.app=c;this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return d.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return d.app.get("/config/"+a)};this.getAll=function(){return d.app.get("/config")};this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";
return d.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(c){var d=this;if(!(this instanceof KidoEmail))return new KidoEmail(c);if(!c)throw"The 'kidoApp' argument is required by the KidoEmail class.";this.app=c;this.send=function(a,b,g,f,c,k,l){if(a&&"object"===typeof a){var h=a;a=h.from;b=h.to;g=h.subject;f=h.bodyText;c=h.bodyHtml;k=h.attachments;l=h.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!k&&c instanceof Array&&(k=c,c=null);h={to:b,from:a};
"string"===typeof g&&0<g.length&&(h.subject=g);"string"===typeof f&&0<f.length&&(h.bodyText=f);"string"===typeof c&&0<c.length&&(h.bodyHtml=c);k instanceof Array&&(h.attachments=k);return d.app.send({url:"/email",type:"POST",data:JSON.stringify(h),timeout:l||9E5})};this.attach=function(a,b,g){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=a.files[0]}if(a instanceof File){var c=
a;a=new FormData;a.append(c.name,c)}if(a instanceof FormData)"function"===typeof b&&(g=b,b=null);else if("string"===typeof a){c=a;a=new FormData;if(!b||"function"===typeof b)throw"if a name was specified, 'data' argument is required.";"string"===typeof b&&(b=new Blob([b],{type:"text/text"}));if(!(b instanceof Blob))throw"'data' argument must be a string, a Blob instance or a File instance.";a.append(c,b,c)}else throw"The argument 'formOrName' is invalid.";var d=new jQuery.Deferred;b=function(a){d.reject(a.currentTarget)};
c=new XMLHttpRequest;c.upload&&"function"===typeof g&&c.upload.addEventListener("progress",g,!1);c.addEventListener("load",function(a){d.resolve(JSON.parse(a.currentTarget.response))},!1);c.addEventListener("error",b,!1);c.addEventListener("abort",b,!1);c.open("POST","/email/attachments");c.send(a);return d.promise()}};Kido.prototype.email=function(){this._email||(this._email=new KidoEmail(this));return this._email};var KidoLogging=function(c){var d=this;if(!(this instanceof KidoLogging))return new KidoLogging(c);if(!c)throw"The 'kidoApp' argument is required by the KidoLogging class.";this.app=c;this.writeVerbose=function(a){return d.write(a,0)};this.writeInfo=function(a){return d.write(a,1)};this.writeWarning=function(a){return d.write(a,2)};this.writeError=function(a){return d.write(a,3)};this.writeCritical=function(a){return d.write(a,4)};this.write=function(a,b){if(!a)throw"'data' argument is requiered.";
if(!b&&0!==b)throw"'level' argument is required.";if(!(0<=b&&4>=b))throw"'level' argument must be an integer number between 0 and 4.";return d.app.send({url:"/logging?level="+b,type:"POST",data:JSON.stringify(a)})};this.query=function(a,b){var c=[];a&&c.push("query="+JSON.stringify(a));b&&c.push("options="+JSON.stringify(b));return d.app.send({url:"/logging"+(0<c.length?"?"+c.join("&"):""),type:"GET"})};this.get=function(a,b,c,f){var e={};b&&(e.level=b);if(a){if(!(a instanceof Date))throw"'Since' argument accepts only null or Date values.";
e.dateTime={$gt:a}}else e.dateTime={$exists:!0};a={$sort:{dateTime:-1}};c&&(a.$skip=c);f&&(a.$limit=f);return d.query(e,a)};this.clear=function(){return d.app.send({url:"/logging",type:"DELETE",dataType:"text"})}};Kido.prototype.logging=function(){this._logging||(this._logging=new KidoLogging(this));return this._logging};var KidoNotifications=function(c){var d=this;if(!(this instanceof KidoNotifications))return new KidoNotifications(c);if(!c)throw"The 'kidoApp' argument is required by the KidoNotifications class.";this.app=c;this.send=function(a,b,c,f,e,k,l){if(!a)throw"'channel' argument is required.";if(!b)throw"'title' argument is required.";b={title:b};"string"===typeof f&&f&&(b.type=f);"string"===typeof c&&c&&(b.text=c);"string"===typeof k&&k&&(b.image=k);"number"===typeof e&&(b.badge=e);l&&(b.param=l);return d.app.send({url:"/notifications/push/local/"+
a,type:"POST",data:JSON.stringify(b)})};this.subscribe=function(a,b,c,f){if(!a)throw"'deviceId' argument is required.";if(!b)throw"'channel' argument is required.";if(!c)throw"'subscriptionId' argument is required.";if(!f)throw"'platform' is required. Use one of: gcm, apns, c2dm, mpns or wns.";return d.app.send({url:"/notifications/subscriptions/local/"+b,type:"POST",data:JSON.stringify({deviceId:a,subscriptionId:c,platform:f})})}};
...
return d.objectSet.invoke({indexes:"indexes/"+a,settings:{type:"DELETE"}})};this.create=function(a,b,c,f,e,k,l,h){if(!a)throw"The 'spec' argument is required to create an index.";b||(b=!1);c||(c=!1);f||(f=!1);e||(e=!1);k||(k=!1);a={spec:a,options:{safe:b,unique:c,sparse:f,background:e,dropDups:k}};l&&"0"!=l.toString()&&(a.options.min=l);h&&"0"!=h.toString()&&(a.options.max=h);l={indexes:"indexes",settings:{type:"POST",data:JSON.stringify(a)}};return d.objectSet.invoke(l)}};
...
a)}).fail(function(a){e.reject(a)});return e}};Kido.prototype.services=function(c){return new KidoService(this,c)};var KidoDatasource=function(c,d){var a=this;if(!(this instanceof KidoDatasource))return new KidoDatasource(c);if(!c)throw"The 'kidoApp' argument is required by the KidoDatasource class.";this.app=c;this.name=d;this._defaults={};this.defaults=function(b){a._defaults=$.extend(a._defaults,b||{});return a};this.query=function(b,c){!c&&$.isNumeric(b)&&(c=b,b=null);var d=$.Deferred(),e=$.extend({},a._defaults,b),e=0<Object.keys(e).length?"?"+$.param(e):"";a.app.get("/api/v2/datasources/"+a.name+e,c?{headers:{timeout:c}}:
{}).done(function(a){if(a.error)return d.reject(a.error);d.resolve(a.data||a)}).fail(function(a){d.reject(a)});return d};this.invoke=function(b,c){!c&&$.isNumeric(b)&&(c=b,b=null);var d=$.Deferred(),e=$.extend({},a._defaults,b);a.app.post("/api/v2/datasources/"+a.name,e,c?{headers:{timeout:c}}:{}).done(function(a){if(a.error)return d.reject(a.error);d.resolve(a.data||a)}).fail(function(a){d.reject(a)});return d}};Kido.prototype.datasources=function(c){return new KidoDatasource(this,c)};
