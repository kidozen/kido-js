...
!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");f.authenticated=!0;h=c;e=d;j=n;a.token='WRAP access_token="'+a.rawToken+'"';var b=decodeURIComponent(a.rawToken).split("&");a.claims=b;for(var l in b){var m=b[l];if(-1<m.indexOf("ExpiresOn")){a.expiresOn=1E3*~~m.split("=")[1]-2E4;break}}return a})})})};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var b=$.extend({},{type:"POST",cache:!0},a);b.data&&!b.contentType&&
