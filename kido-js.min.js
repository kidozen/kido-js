...
f.authenticated=!0;h=c;e=d;k=p;a.token='WRAP access_token="'+a.rawToken+'"';var b=decodeURIComponent(a.rawToken).split("&");a.claims=b;for(var m in b){var n=b[m];if(-1<n.indexOf("ExpiresOn")){a.expiresOn=1E3*~~n.split("=")[1]-2E4;break}}return a}):$.Deferred().reject("Unable to get a token from IDP")})}):$.Deferred().reject("No need to authenticate to this Web App")};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var b=$.extend({},{type:"POST",
cache:!0},a);b.data&&!b.contentType&&(b.contentType="application/json");return f.active&&f.authenticated?f.token.pipe(function(a){return a.expiresOn<(new Date).getTime()?f.authenticate(h,e,k):a}).pipe(function(a){b.url=f.url+b.url;b.headers=b.headers||{};b.headers.authorization=a.token;return $.ajax(b)}):$.ajax(b)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return f.send(b)};this.post=function(a,b,c){c=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},c||{});return f.send(c)};
this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return f.send(b)}};var KidoConfig=function(c){var d=this;if(!(this instanceof KidoConfig))return new KidoConfig(c);if(!c)throw"The 'kidoApp' argument is required by the KidoConfig class.";this.app=c;this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return d.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return d.app.get("/config/"+a)};this.getAll=function(){return d.app.get("/config")};this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";
return d.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(c){var d=this;if(!(this instanceof KidoEmail))return new KidoEmail(c);if(!c)throw"The 'kidoApp' argument is required by the KidoEmail class.";this.app=c;this.send=function(a,b,f,c,e,k,l){if(a&&"object"===typeof a){var g=a;a=g.from;b=g.to;f=g.subject;c=g.bodyText;e=g.bodyHtml;k=g.attachments;l=g.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!k&&e instanceof Array&&(k=e,e=null);g={to:b,from:a};
"string"===typeof f&&0<f.length&&(g.subject=f);"string"===typeof c&&0<c.length&&(g.bodyText=c);"string"===typeof e&&0<e.length&&(g.bodyHtml=e);k instanceof Array&&(g.attachments=k);return d.app.send({url:"/email",type:"POST",data:JSON.stringify(g),timeout:l||9E5})};this.attach=function(a,b,f){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=a.files[0]}if(a instanceof File){var c=
...
e.dateTime={$gt:a}}else e.dateTime={$exists:!0};a={$sort:{dateTime:-1}};c&&(a.$skip=c);h&&(a.$limit=h);return d.query(e,a)};this.clear=function(){return d.app.send({url:"/logging",type:"DELETE",dataType:"text"})}};Kido.prototype.logging=function(){this._logging||(this._logging=new KidoLogging(this));return this._logging};var KidoNotifications=function(c){var d=this;if(!(this instanceof KidoNotifications))return new KidoNotifications(c);if(!c)throw"The 'kidoApp' argument is required by the KidoNotifications class.";this.app=c;this.send=function(a,b,c,h,e,k,l){if(!a)throw"'channel' argument is required.";if(!b)throw"'title' argument is required.";b={title:b};"string"===typeof h&&h&&(b.type=h);"string"===typeof c&&c&&(b.text=c);"string"===typeof k&&k&&(b.image=k);"number"===typeof e&&(b.badge=e);l&&(b.param=l);return d.app.send({url:"/notifications/push/local/"+
...
return d.objectSet.invoke({indexes:"indexes/"+a,settings:{type:"DELETE"}})};this.create=function(a,b,c,h,e,k,l,g){if(!a)throw"The 'spec' argument is required to create an index.";b||(b=!1);c||(c=!1);h||(h=!1);e||(e=!1);k||(k=!1);a={spec:a,options:{safe:b,unique:c,sparse:h,background:e,dropDups:k}};l&&"0"!=l.toString()&&(a.options.min=l);g&&"0"!=g.toString()&&(a.options.max=g);l={indexes:"indexes",settings:{type:"POST",data:JSON.stringify(a)}};return d.objectSet.invoke(l)}};
