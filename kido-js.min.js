...
a.user).replace("[Password]",a.pass).replace("[ApplyTo]",a.scope),headers:{"Content-Type":"application/soap+xml; charset=utf-8"}})}function b(a){return $.ajax({url:a.endpoint,data:{wrap_name:a.user,wrap_password:a.pass,wrap_scope:a.scope},type:"POST",dataType:"text"})}if(!(this instanceof Kido))return new Kido;var h=this;this.name=c||"local";this.marketplace=d;this.local="local"===this.name;this.authenticated=(this.active=!!d)?!1:!0;this.active&&(this.authConfig=$.ajax({url:this.marketplace+"/publicapi/apps?name="+
this.name}).pipe(function(a){if(!a||!a.length)return $.Deferred().reject("Application configuration not found.");h.url=a[0].url.substring(0,a[0].url.length-1);return a[0].authConfig}));this.authenticate=function(g,e,c){return!this.active?$.Deferred().reject("No need to authenticate to this Web App"):this.token=this.authConfig.pipe(function(d){var f=d.identityProviders;c=c||Object.keys(f)[0];f=f[c];if(!f)return $.Deferred().reject("Invalid Identity Provider");if(!f.protocol)return $.Deferred().reject("Invalid Identity Provider Protocol");
var l;if("wrapv0.9"===f.protocol.toLowerCase())l=b;else if("ws-trust"===f.protocol.toLowerCase())l=a;else return $.Deferred().reject("Protocol not supported yet.");return l({user:g,pass:e,scope:d.authServiceScope,endpoint:f.endpoint}).pipe(function(a){a=(/<Assertion(.*)<\/Assertion>/.exec(a)||[])[0];return!a?$.Deferred().reject("Unable to get a token from IDP"):$.ajax({url:d.authServiceEndpoint,type:"POST",data:{wrap_assertion:a,wrap_scope:d.applicationScope,wrap_assertion_format:"SAML"}}).pipe(function(a){h.authenticated=
!0;a.token='WRAP access_token="'+a.rawToken+'"';var b=decodeURIComponent(a.rawToken).split("&");a.claims=b;for(var e in b){var g=b[e];if(-1<g.indexOf("ExpiresOn")){a.expiresOn=~~g.split("=")[1]-2E4;break}}return a})})})};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var b=$.extend({},{type:"POST",cache:!0},a);b.data&&!b.contentType&&(b.contentType="application/json");return h.active&&h.authenticated?h.token.pipe(function(a){b.url=h.url+
b.url;b.headers=b.headers||{};b.headers.authorization=a.token;return $.ajax(b)}):$.ajax(b)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return h.send(b)};this.post=function(a,b,c){c=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},c||{});return h.send(c)};this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return h.send(b)}};var KidoConfig=function(c){var d=this;if(!(this instanceof KidoConfig))return new KidoConfig(c);if(!c)throw"The 'kidoApp' argument is required by the KidoConfig class.";this.app=c;this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return d.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return d.app.get("/config/"+a)};this.getAll=function(){return d.app.get("/config")};this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";
return d.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(c){var d=this;if(!(this instanceof KidoEmail))return new KidoEmail(c);if(!c)throw"The 'kidoApp' argument is required by the KidoEmail class.";this.app=c;this.send=function(a,b,h,c,e,j,k){if(a&&"object"===typeof a){var f=a;a=f.from;b=f.to;h=f.subject;c=f.bodyText;e=f.bodyHtml;j=f.attachments;k=f.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!j&&e instanceof Array&&(j=e,e=null);f={to:b,from:a};
"string"===typeof h&&0<h.length&&(f.subject=h);"string"===typeof c&&0<c.length&&(f.bodyText=c);"string"===typeof e&&0<e.length&&(f.bodyHtml=e);j instanceof Array&&(f.attachments=j);return d.app.send({url:"/email",type:"POST",data:JSON.stringify(f),timeout:k||9E5})};this.attach=function(a,b,h){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=a.files[0]}if(a instanceof File){var c=
a;a=new FormData;a.append(c.name,c)}if(a instanceof FormData)"function"===typeof b&&(h=b,b=null);else if("string"===typeof a){c=a;a=new FormData;if(!b||"function"===typeof b)throw"if a name was specified, 'data' argument is required.";"string"===typeof b&&(b=new Blob([b],{type:"text/text"}));if(!(b instanceof Blob))throw"'data' argument must be a string, a Blob instance or a File instance.";a.append(c,b,c)}else throw"The argument 'formOrName' is invalid.";var d=new jQuery.Deferred;b=function(a){d.reject(a.currentTarget)};
c=new XMLHttpRequest;c.upload&&"function"===typeof h&&c.upload.addEventListener("progress",h,!1);c.addEventListener("load",function(a){d.resolve(JSON.parse(a.currentTarget.response))},!1);c.addEventListener("error",b,!1);c.addEventListener("abort",b,!1);c.open("POST","/email/attachments");c.send(a);return d.promise()}};Kido.prototype.email=function(){this._email||(this._email=new KidoEmail(this));return this._email};var KidoLogging=function(c){var d=this;if(!(this instanceof KidoLogging))return new KidoLogging(c);if(!c)throw"The 'kidoApp' argument is required by the KidoLogging class.";this.app=c;this.writeVerbose=function(a){return this.write(a,0)};this.writeInfo=function(a){return this.write(a,1)};this.writeWarning=function(a){return this.write(a,2)};this.writeError=function(a){return this.write(a,3)};this.writeCritical=function(a){return this.write(a,4)};this.write=function(a,b){if(!a)throw"'data' argument is requiered.";
if(!b&&0!==b)throw"'level' argument is required.";if(!(0<=b&&4>=b))throw"'level' argument must be an integer number between 0 and 4.";return d.app.send({url:"/logging?level="+b,type:"POST",data:JSON.stringify(a)})};this.query=function(a,b){var c=[];a&&c.push("query="+JSON.stringify(a));b&&c.push("options="+JSON.stringify(b));return d.app.send({url:"/logging"+(0<c.length?"?"+c.join("&"):""),type:"GET"})};this.get=function(a,b,c,g){var e={};b&&(e.level=b);if(a){if(!(a instanceof Date))throw"'Since' argument accepts only null or Date values.";
e.dateTime={$gt:a}}else e.dateTime={$exists:!0};a={$sort:{dateTime:-1}};c&&(a.$skip=c);g&&(a.$limit=g);return d.query(e,a)};this.clear=function(){return d.app.send({url:"/logging",type:"DELETE",dataType:"text"})}};Kido.prototype.logging=function(){this._logging||(this._logging=new KidoLogging(this));return this._logging};var KidoNotifications=function(c){var d=this;if(!(this instanceof KidoNotifications))return new KidoNotifications(c);if(!c)throw"The 'kidoApp' argument is required by the KidoNotifications class.";this.app=c;this.send=function(a,b,c,g,e,j,k){if(!a)throw"'channel' argument is required.";if(!b)throw"'title' argument is required.";b={title:b};"string"===typeof g&&g&&(b.type=g);"string"===typeof c&&c&&(b.text=c);"string"===typeof j&&j&&(b.image=j);"number"===typeof e&&(b.badge=e);k&&(b.param=k);return d.app.send({url:"/notifications/push/local/"+
a,type:"POST",data:JSON.stringify(b)})};this.subscribe=function(a,b,c,g){if(!a)throw"'deviceId' argument is required.";if(!b)throw"'channel' argument is required.";if(!c)throw"'subscriptionId' argument is required.";if(!g)throw"'platform' is required. Use one of: gcm, apns, c2dm, mpns or wns.";return d.app.send({url:"/notifications/subscriptions/local/"+b,type:"POST",data:JSON.stringify({deviceId:a,subscriptionId:c,platform:g})})}};
...
return d.objectSet.invoke({indexes:"indexes/"+a,settings:{type:"DELETE"}})};this.create=function(a,b,c,g,e,j,k,f){if(!a)throw"The 'spec' argument is required to create an index.";b||(b=!1);c||(c=!1);g||(g=!1);e||(e=!1);j||(j=!1);a={spec:a,options:{safe:b,unique:c,sparse:g,background:e,dropDups:j}};k&&"0"!=k.toString()&&(a.options.min=k);f&&"0"!=f.toString()&&(a.options.max=f);k={indexes:"indexes",settings:{type:"POST",data:JSON.stringify(a)}};return d.objectSet.invoke(k)}};
