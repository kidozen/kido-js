...
"_blank","location=yes");l.isNative?(e.addEventListener("loadstop",function(){e.executeScript({code:"document.title;"},function(a){a=a[0];if(-1!==a.indexOf("Success payload=")){try{var c=m(a);l.authenticated=!0;b.resolve(c)}catch(h){b.reject("Unable to retrieve KidoZen token.")}e.close()}})}),e.addEventListener("loaderror",function(){b.reject("InAppBrowser error loading page.");e.close()}),e.addEventListener("exit",function(){b.reject("InAppBrowser has exited.")})):window.addEventListener("message",
function(c){var h=document.createElement("a");h.href=a.signInUrl;if(c.origin!==h.protocol+"//"+h.host)return b.reject("Unable to retrieve KidoZen token.");try{var d=m(c.data);l.authenticated=!0;b.resolve(d)}catch(f){b.reject("Unable to retrieve KidoZen token.")}e.close()},!1);return b.promise()}function g(a,b,c,d,f){var g;if("wrapv0.9"===f.protocol.toLowerCase())g=h;else if("ws-trust"===f.protocol.toLowerCase())g=e;else return $.Deferred().reject("Protocol not supported yet.");return g({user:b,pass:c,
scope:a.authServiceScope,endpoint:f.endpoint}).then(function(e){return(e=(/<Assertion(.*)<\/Assertion>/.exec(e)||[])[0])?$.ajax({url:a.authServiceEndpoint,type:"POST",data:{wrap_assertion:e,wrap_scope:a.applicationScope,wrap_assertion_format:"SAML"}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");l.authenticated=!0;x=b;y=c;z=d;return k(a)}):$.Deferred().reject("Unable to get a token from IDP")})}function m(a){if(-1===a.indexOf("Success payload="))throw"Unable to retrieve KidoZen token.";
...
if("undefined"===typeof $.fn||"undefined"===typeof $.fn.jquery)throw"Could not determine jQuery version.";var D=$.fn.jquery.split(".");if(2>parseInt(D[0])&&8>parseInt(D[1]))throw"jQuery 1.8 or above is required to use the Kido SDK.";if("undefined"!==typeof c)if(-1===c.indexOf("://"))c="https://"+c;else if(0!==c.indexOf("http://")&&0!==c.indexOf("https://"))throw"Marketplace url must begin with https://";var l=this,x=null,y=null,z=null,C=null;this.name=d||"local";this.marketplace=c;this.local="local"===
this.name;this.authenticated=(this.hosted=!c)?!0:!1;this.isNative=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://");"object"===typeof a&&("object"===typeof a.token&&(this.authenticated=!0,this.url=c,this.token=$.Deferred().resolve(k(a.token))),"string"===typeof a.username&&(x=a.username),"string"===typeof a.password&&(y=a.password),"string"===typeof a.provider&&(z=a.provider),"string"===typeof a.secretKey&&(C=a.secretKey));this.hosted||(this.getConfig=$.ajax({url:this.marketplace+
"/publicapi/apps?name="+this.name}).then(function(a){if(!a||!a.length)return $.Deferred().reject("Application configuration not found.");l.url=a[0].url.substring(0,a[0].url.length-1);return a[0]},function(){return $.Deferred().reject("Unable to retrieve application configuration. Either there is no internet connection or the marketplace url is invalid.")}));this.authenticate=function(){if(l.hosted)return $.Deferred().reject("No need to authenticate to this Web App");var a=arguments;l.token=l.getConfig.then(function(b){if(0===
a.length)return f(b.authConfig);var e=b.authConfig.identityProviders,c=a[2]||Object.keys(e)[0];return(e=e[c])?e.protocol?g(b.authConfig,a[0],a[1],a[2],e):$.Deferred().reject("Invalid Identity Provider Protocol"):$.Deferred().reject("Invalid Identity Provider")});return l.token};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var e=$.extend({},{type:"POST",cache:!0},a);e.data&&!e.contentType&&(e.contentType="application/json");return!l.hosted&&
l.authenticated?l.token.then(function(a){return a.expiresOn<(new Date).getTime()?x&&y&&z?l.authenticate(x,y,z):C?l.getConfig.then(function(b){return $.ajax({url:b.authConfig.oauthTokenEndpoint,type:"POST",data:{grant_type:"refresh_token",client_id:b.domain,client_secret:C,scope:b.authConfig.applicationScope,refresh_token:a.refresh_token}})}):l.authenticate():a}).then(function(a){e.url=l.url+e.url;e.headers=e.headers||{};e.headers.authorization=a.token;return b(e)}):b(e)};this.get=function(a,b){b=
$.extend({url:a,type:"GET"},b||{});return l.send(b)};this.post=function(a,b,e){e=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},e||{});return l.send(e)};this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return l.send(b)}};var KidoConfig=function(d){if(!(this instanceof KidoConfig))return new KidoConfig(d);if(!d)throw"The 'kidoApp' argument is required by the KidoConfig class.";var c=this;this.app=d;this.SERVICE_NAME="config";this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return c.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return c.app.get("/config/"+a)};this.getAll=function(){return c.app.get("/config")};
this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";return c.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(d){if(!(this instanceof KidoEmail))return new KidoEmail(d);if(!d)throw"The 'kidoApp' argument is required by the KidoEmail class.";var c=this;this.app=d;this.SERVICE_NAME="email";this.send=function(a,b,e,h,d,g,m){if(a&&"object"===typeof a){var k=a;a=k.from;b=k.to;e=k.subject;h=k.bodyText;d=k.bodyHtml;g=k.attachments;m=k.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!g&&d instanceof Array&&
(g=d,d=null);k={to:b,from:a};"string"===typeof e&&0<e.length&&(k.subject=e);"string"===typeof h&&0<h.length&&(k.bodyText=h);"string"===typeof d&&0<d.length&&(k.bodyHtml=d);g instanceof Array&&(k.attachments=g);return c.app.send({url:"/email",type:"POST",data:JSON.stringify(k),timeout:m||9E5})};this.attach=function(a,b,e){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=
...
d.del(f).then(function(){return e.del(f)})};this.getNewIdFromDictionary=function(a){return e.get(a,!0).then(function(a){return a.new_id})};this.ajax=function(a){if(!a)throw"The 'settings' argument is required.";var b=a.data,d=a.kidoService.service,e=a.kidoService.collection,f=a.kidoService.objectId,g=a.kidoService.query,h=a.kidoService.caching,m=a.kidoService.queueing,n=a.type.toLowerCase(),s="get"===n&&f,v="get"===n&&!f,B="post"===n&&b,t="put"===n&&b,u="delete"===n&&f,w="delete"===n&&!f,n=$.ajax(a),
...
u?n.then(function(){return p.del(f?f:A)}):w&&n.then(function(){return p.drop()})}n.fail(function(d){if(0!==d.status)return r.reject(d);c.startCheckingConnectivity();if(m&&!h)if(s||v)r.reject(d);else{var e=(-1*$.now()).toString();c.addPendingRequest(e,a).then(function(){r.reject(d)})}else h?(e=function(b){if(m&&(B||t||u||w)){var d=w?(-1*$.now()).toString():u?f:t?A:b._id;c.addPendingRequest(d,a).then(function(){r.resolve(b)})}else r.resolve(b)},v?p.query(g).then(e):s?p.get(f).then(e):B||t?p.persist(b).then(e):
