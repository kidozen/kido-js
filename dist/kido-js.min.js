...
!a.access_token&&!a.rawToken)return h.authenticated=!1,$.Deferred().reject("Unable to retrieve KidoZen token.");a.refresh_token=b;return k(a)})});return h.token}return a}).then(function(a){e.url=h.url+e.url;e.headers=e.headers||{};e.headers.authorization=a.token;return b(e)}):b(e)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return h.send(b)};this.post=function(a,b,e){e=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},e||{});return h.send(e)};this.del=function(a,b){b=$.extend({url:a,
type:"DELETE"},b||{});return h.send(b)}};var KidoConfig=function(d){if(!(this instanceof KidoConfig))return new KidoConfig(d);if(!d)throw"The 'kidoApp' argument is required by the KidoConfig class.";var c=this;this.app=d;this.SERVICE_NAME="config";this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return c.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return c.app.get("/config/"+a)};this.getAll=function(){return c.app.get("/config")};
