...
scope:a.authServiceScope,endpoint:f.endpoint}).then(function(e){return(e=(/<Assertion(.*)<\/Assertion>/.exec(e)||[])[0])?$.ajax({url:a.authServiceEndpoint,type:"POST",data:{wrap_assertion:e,wrap_scope:a.applicationScope,wrap_assertion_format:"SAML"}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");m.authenticated=!0;w=b;x=c;y=d;return k(a)}):$.Deferred().reject("Unable to get a token from IDP")})}function l(a){if(-1===a.indexOf("Success payload="))throw"Unable to retrieve KidoZen token.";
...
if("undefined"===typeof $.fn||"undefined"===typeof $.fn.jquery)throw"Could not determine jQuery version.";var C=$.fn.jquery.split(".");if(2>parseInt(C[0])&&8>parseInt(C[1]))throw"jQuery 1.8 or above is required to use the Kido SDK.";if("undefined"!==typeof c)if(-1===c.indexOf("://"))c="https://"+c;else if(0!==c.indexOf("http://")&&0!==c.indexOf("https://"))throw"Marketplace url must begin with https://";var m=this,w=null,x=null,y=null;this.name=d||"local";this.marketplace=c;this.local="local"===this.name;
this.authenticated=(this.hosted=!c)?!0:!1;this.isNative=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://");"object"===typeof a&&("object"===typeof a.token&&(this.authenticated=!0,this.url=c,this.token=$.Deferred().resolve(k(a.token))),"string"===typeof a.username&&(w=a.username),"string"===typeof a.password&&(x=a.password),"string"===typeof a.provider&&(y=a.provider));this.hosted||(this.authConfig=$.ajax({url:this.marketplace+"/publicapi/apps?name="+this.name}).then(function(a){if(!a||
!a.length)return $.Deferred().reject("Application configuration not found.");m.url=a[0].url.substring(0,a[0].url.length-1);return a[0].authConfig},function(){return $.Deferred().reject("Unable to retrieve application configuration. Either there is no internet connection or the marketplace url is invalid.")}));this.authenticate=function(){if(m.hosted)return $.Deferred().reject("No need to authenticate to this Web App");var a=arguments;m.token=m.authConfig.then(function(b){if(0===a.length)return f(b);
var e=b.identityProviders,c=a[2]||Object.keys(e)[0];return(e=e[c])?e.protocol?g(b,a[0],a[1],a[2],e):$.Deferred().reject("Invalid Identity Provider Protocol"):$.Deferred().reject("Invalid Identity Provider")});return m.token};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var e=$.extend({},{type:"POST",cache:!0},a);e.data&&!e.contentType&&(e.contentType="application/json");return!m.hosted&&m.authenticated?m.token.then(function(a){return a.expiresOn<
(new Date).getTime()?w&&x&&y?m.authenticate(w,x,y):m.authenticate():a}).then(function(a){e.url=m.url+e.url;e.headers=e.headers||{};e.headers.authorization=a.token;return b(e)}):b(e)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return m.send(b)};this.post=function(a,b,e){e=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},e||{});return m.send(e)};this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return m.send(b)}};var KidoConfig=function(d){if(!(this instanceof KidoConfig))return new KidoConfig(d);if(!d)throw"The 'kidoApp' argument is required by the KidoConfig class.";var c=this;this.app=d;this.SERVICE_NAME="config";this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return c.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return c.app.get("/config/"+a)};this.getAll=function(){return c.app.get("/config")};
this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";return c.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(d){if(!(this instanceof KidoEmail))return new KidoEmail(d);if(!d)throw"The 'kidoApp' argument is required by the KidoEmail class.";var c=this;this.app=d;this.SERVICE_NAME="email";this.send=function(a,b,e,h,d,g,l){if(a&&"object"===typeof a){var k=a;a=k.from;b=k.to;e=k.subject;h=k.bodyText;d=k.bodyHtml;g=k.attachments;l=k.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!g&&d instanceof Array&&
(g=d,d=null);k={to:b,from:a};"string"===typeof e&&0<e.length&&(k.subject=e);"string"===typeof h&&0<h.length&&(k.bodyText=h);"string"===typeof d&&0<d.length&&(k.bodyHtml=d);g instanceof Array&&(k.attachments=g);return c.app.send({url:"/email",type:"POST",data:JSON.stringify(k),timeout:l||9E5})};this.attach=function(a,b,e){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=
...
d.del(f).then(function(){return e.del(f)})};this.getNewIdFromDictionary=function(a){return e.get(a,!0).then(function(a){return a.new_id})};this.ajax=function(a){if(!a)throw"The 'settings' argument is required.";var b=a.data,d=a.kidoService.service,e=a.kidoService.collection,f=a.kidoService.objectId,g=a.kidoService.query,h=a.kidoService.caching,l=a.kidoService.queueing,n=a.type.toLowerCase(),s="get"===n&&f,v="get"===n&&!f,B="post"===n&&b,t="put"===n&&b,u="delete"===n&&f,z="delete"===n&&!f,n=$.ajax(a),
r=$.Deferred();if(h){var p=c.app.localStorage().collection(d+"."+e),q=b?JSON.parse(b):null,A=q?q._id:null,d=f&&0>parseInt(f)||A&&0>parseInt(A);if(s||v)a.timeout=5E3;if(d){if(s)return p.get(f);if(u)return p.del(f).then(function(){return c.removePendingRequest(f)});t&&(a.type="POST",delete q._id,a.data=JSON.stringify(q),f=null)}s||v?n.then(function(a){return p.persist(a)}):B||t?n.then(function(a){q._id=a._id?a._id:q._id?q._id:null;q._metadata=a._metadata?a._metadata:q._metadata?q._metadata:null;return p.persist(q)}):
u?n.then(function(){return p.del(f?f:A)}):z&&n.then(function(){return p.drop()})}n.fail(function(d){if(0!==d.status)return r.reject(d);c.startCheckingConnectivity();if(l&&!h)if(s||v)r.reject(d);else{var e=(-1*$.now()).toString();c.addPendingRequest(e,a).then(function(){r.reject(d)})}else h?(e=function(b){if(l&&(B||t||u||z)){var d=z?(-1*$.now()).toString():u?f:t?A:b._id;c.addPendingRequest(d,a).then(function(){r.resolve(b)})}else r.resolve(b)},v?p.query(g).then(e):s?p.get(f).then(e):B||t?p.persist(b).then(e):
u?p.del(f).then(e):z&&p.drop().then(e)):r.reject(d)});n.done(function(a){r.resolve(a)});return r.promise()}};Kido.prototype.offline=function(){this._offline||(this._offline=new KidoOffline(this));return this._offline};
