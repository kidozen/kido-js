...
"_blank","location=yes");g.isNative?(e.addEventListener("loadstop",function(){e.executeScript({code:"document.title;"},function(a){a=a[0];if(-1!==a.indexOf("Success payload=")){try{var c=m(a);g.authenticated=!0;b.resolve(c)}catch(l){b.reject("Unable to retrieve KidoZen token.")}e.close()}})}),e.addEventListener("loaderror",function(){b.reject("InAppBrowser error loading page.");e.close()}),e.addEventListener("exit",function(){b.reject("InAppBrowser has exited.")})):window.addEventListener("message",
function(c){var l=document.createElement("a");l.href=a.signInUrl;if(c.origin!==l.protocol+"//"+l.host)return b.reject("Unable to retrieve KidoZen token.");try{var d=m(c.data);g.authenticated=!0;b.resolve(d)}catch(f){b.reject("Unable to retrieve KidoZen token.")}e.close()},!1);return b.promise()}function h(a,b,c,d,f){var h;if("wrapv0.9"===f.protocol.toLowerCase())h=l;else if("ws-trust"===f.protocol.toLowerCase())h=e;else return $.Deferred().reject("Protocol not supported yet.");return h({user:b,pass:c,
scope:a.authServiceScope,endpoint:f.endpoint}).then(function(e){return(e=(/<Assertion(.*)<\/Assertion>/.exec(e)||[])[0])?$.ajax({url:a.authServiceEndpoint,type:"POST",data:{wrap_assertion:e,wrap_scope:a.applicationScope,wrap_assertion_format:"SAML"}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");g.authenticated=!0;y=b;r=c;z=d;return k(a)}):$.Deferred().reject("Unable to get a token from IDP")})}function m(a){if(-1===a.indexOf("Success payload="))throw"Unable to retrieve KidoZen token.";
...
if("undefined"===typeof $.fn||"undefined"===typeof $.fn.jquery)throw"Could not determine jQuery version.";var D=$.fn.jquery.split(".");if(2>parseInt(D[0])&&8>parseInt(D[1]))throw"jQuery 1.8 or above is required to use the Kido SDK.";if("undefined"!==typeof c)if(-1===c.indexOf("://"))c="https://"+c;else if(0!==c.indexOf("http://")&&0!==c.indexOf("https://"))throw"Marketplace url must begin with https://";var g=this,y=null,r=null,z=null,C=null;this.name=d||"local";this.marketplace=c;this.local="local"===
this.name;this.authenticated=(this.hosted=!c)?!0:!1;this.isNative=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://");this.useHttpCompression=!1;"object"===typeof a&&("object"===typeof a.token&&(this.inheritedToken=a.token),"string"===typeof a.username&&(y=a.username),"string"===typeof a.password&&(r=a.password),"string"===typeof a.provider&&(z=a.provider),"string"===typeof a.secretKey&&(C=a.secretKey),this.useHttpCompression=a.useHttpCompression||!1);this.hosted||(this.getConfig=
$.ajax({url:this.marketplace+"/publicapi/apps?name="+this.name}).then(function(a){if(!a||!a.length)return $.Deferred().reject("Application configuration not found.");g.url=a[0].url.substring(0,a[0].url.length-1);return a[0]},function(){return $.Deferred().reject("Unable to retrieve application configuration. Either there is no internet connection or the marketplace url is invalid.")}));this.authenticate=function(){if(g.hosted)return $.Deferred().resolve("No need to authenticate to this Web App");
var a=arguments;g.token=g.getConfig.then(function(b){if(0===a.length&&g.inheritedToken)return g.authenticated=!0,k(g.inheritedToken);if(0===a.length)return f(b.authConfig);var e=b.authConfig.identityProviders,c=a[2]||Object.keys(e)[0];return(e=e[c])?e.protocol?h(b.authConfig,a[0],a[1],a[2],e):$.Deferred().reject("Invalid Identity Provider Protocol"):$.Deferred().reject("Invalid Identity Provider")});return g.token};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",
error:"Invalid URL"});var e=$.extend({},{type:"POST",cache:!0,headers:{"X-Kido-SDK":"JS","X-Kido-SDK-Version":KIDO_SDK_VERSION||"UNKNOWN"}},a);g.useHttpCompression&&(e.headers["Accept-Encoding"]="gzip, deflate");e.data&&!e.contentType&&(e.contentType="application/json");return!g.hosted&&g.authenticated?g.token.then(function(a){if(a.expiresOn<(new Date).getTime()){if(y&&r&&z)return g.authenticate(y,r,z);if(!C)return g.authenticate();var b=a.refresh_token;g.token=g.getConfig.then(function(a){return $.ajax({url:a.authConfig.oauthTokenEndpoint,
type:"POST",data:{grant_type:"refresh_token",client_id:a.domain,client_secret:C,scope:a.authConfig.applicationScope,refresh_token:b}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return g.authenticated=!1,$.Deferred().reject("Unable to retrieve KidoZen token.");a.refresh_token=b;return k(a)})});return g.token}return a}).then(function(a){e.url=g.url+e.url;e.headers=e.headers||{};e.headers.authorization=a.token;return b(e)}):b(e)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});
return g.send(b)};this.post=function(a,b,e){e=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},e||{});return g.send(e)};this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return g.send(b)}};var KidoConfig=function(d){if(!(this instanceof KidoConfig))return new KidoConfig(d);if(!d)throw"The 'kidoApp' argument is required by the KidoConfig class.";var c=this;this.app=d;this.SERVICE_NAME="config";this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return c.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return c.app.get("/config/"+a)};this.getAll=function(){return c.app.get("/config")};
this.del=function(a){if(!a)throw"The config key 'name' is required to delete a value.";return c.app.del("/config/"+a)}};Kido.prototype.config=function(){this._config||(this._config=new KidoConfig(this));return this._config};var KidoEmail=function(d){if(!(this instanceof KidoEmail))return new KidoEmail(d);if(!d)throw"The 'kidoApp' argument is required by the KidoEmail class.";var c=this;this.app=d;this.SERVICE_NAME="email";this.send=function(a,b,e,l,f,d,m){if(a&&"object"===typeof a){var k=a;a=k.from;b=k.to;e=k.subject;l=k.bodyText;f=k.bodyHtml;d=k.attachments;m=k.timeout}if(!a)throw"The 'from' argument is required to send an email.";if(!b)throw"The 'to' argument is required to send an email.";!d&&f instanceof Array&&
(d=f,f=null);k={to:b,from:a};"string"===typeof e&&0<e.length&&(k.subject=e);"string"===typeof l&&0<l.length&&(k.bodyText=l);"string"===typeof f&&0<f.length&&(k.bodyHtml=f);d instanceof Array&&(k.attachments=d);return c.app.send({url:"/email",type:"POST",data:JSON.stringify(k),timeout:m||9E5})};this.attach=function(a,b,e){a instanceof HTMLFormElement&&(a=new FormData(a));if(a instanceof HTMLInputElement){if(!a.type||"file"!==a.type.toLowerCase())throw"HTML Input element must be of type 'file'.";a=
...
Kido.prototype.queues=function(){this._queues||(this._queues=new KidoQueues(this));return this._queues};var KidoSecurity=function(d){if(!(this instanceof KidoSecurity))return new KidoSecurity(d);if(!d)throw"The 'kidoApp' argument is required by the KidoSecurity class.";var c=this;this.app=d;this.SERVICE_NAME="security";this.getLoggedInUser=function(){return c.app.get("/user").then(function(a){var b={claims:{}};a.forEach(function(a){if(-1!==a.type.indexOf("http://schemas.kidozen.com/")){var c=a.type.substring(a.type.lastIndexOf("/")+1);b[c]=a.value}else b.claims[a.type]=a.value});return b})};this.getOriginalToken=
function(){return c.app.get("/user/original-token")};this.getJWT=function(a){return c.app.get("/api/v3/delegation/token"+(a?"?gzip=true":""))}};Kido.prototype.security=function(){this._security||(this._security=new KidoSecurity(this));return this._security};var KidoSms=function(d,c){if(!(this instanceof KidoSms))return new KidoSms(d);if(!d)throw"The 'kidoApp' argument is required by the KidoSms class.";c||(c={queueing:!1});var a=this;this.app=d;this.SERVICE_NAME="sms";this.caching=!1;this.queueing=c.queueing||!1;this.send=function(b,e){if(!b)throw"The 'to' argument is required to send an sms.";if(!e)throw"The 'message' argument is required to send an sms.";var c={url:"/sms?to="+encodeURIComponent(b)+"&message="+encodeURIComponent(e),type:"POST"};c.kidoService=
...
this.del=function(a){if(!a)throw"The 'name' argument is required to delete an object set index.";return c.objectSet.invoke({indexes:"indexes/"+a,settings:{type:"DELETE"}})};this.create=function(a,b,e,d,f,h,m,k){if(!a)throw"The 'spec' argument is required to create an index.";b||(b=!1);e||(e=!1);d||(d=!1);f||(f=!1);h||(h=!1);a={spec:a,options:{safe:b,unique:e,sparse:d,background:f,dropDups:h}};m&&"0"!=m.toString()&&(a.options.min=m);k&&"0"!=k.toString()&&(a.options.max=k);m={indexes:"indexes",settings:{type:"POST",
...
f.resolve(a.data||a)}).fail(function(a){f.reject(a)});return f}};Kido.prototype.services=function(d){return new KidoService(this,d)};var KidoDatasource=function(d,c,a){if(!(this instanceof KidoDatasource))return new KidoDatasource(d,c);if(!d)throw"The 'kidoApp' argument is required by the KidoDatasource class.";if(!c)throw"The 'name' argument is required by the KidoDatasource class.";a||(a={caching:!1,queueing:!1,timeout:null,useHttpCompression:!1});var b=this;this.app=d;this.SERVICE_NAME="datasources";this.name=c;this._defaults={};this.caching=a.caching||!1;this.queueing=a.queueing||!1;this.timeout=a.timeout||null;this.useHttpCompression=
a.useHttpCompression||!1;this.defaults=function(a){b._defaults=$.extend(b._defaults,a||{});return b};this.query=function(a){var c=$.Deferred();a=$.extend({},b._defaults,a);a=0<Object.keys(a).length?"?"+$.param(a):"";var d={headers:{}};b.timeout&&(d.headers.timeout=b.timeout);b.useHttpCompression&&(d.headers["Accept-Encoding"]="gzip, deflate");d.kidoService={service:b.SERVICE_NAME,collection:b.name,caching:b.caching,queueing:!1};b.app.get("/api/v2/datasources/"+b.name+a,d).done(function(a){if(a.error)return c.reject(a.error);
c.resolve(a.data||a)}).fail(function(a){c.reject(a)});return c};this.invoke=function(a){var c=$.Deferred();a=$.extend({},b._defaults,a);var d={headers:{}};b.timeout&&(d.headers.timeout=b.timeout);b.useHttpCompression&&(d.headers["Accept-Encoding"]="gzip, deflate");d.kidoService={service:b.SERVICE_NAME,collection:b.name,caching:!1,queueing:b.queueing};b.app.post("/api/v2/datasources/"+b.name,a,d).done(function(a){if(a.error)return c.reject(a.error);c.resolve(a.data||a)}).fail(function(a){c.reject(a)});
return c}};Kido.prototype.datasources=function(d,c){return new KidoDatasource(this,d,c)};var KidoLocalStorage=function(d){if(!(this instanceof KidoLocalStorage))return new KidoLocalStorage(d);if(!localforage)throw"KidoLocalStorage needs Mozilla LocalForage to be able to work.";this.app=d;this.SERVICE_NAME="localstorage";localforage.config({name:"kidozen",storeName:"kidozen"});localforage.setItemAsync=function(c,a){var b=$.Deferred();localforage.setItem(c,a).then(function(a){setTimeout(function(){b.resolve(a)},50)},function(){b.reject()});return b.promise()};localforage.getItemAsync=function(c){var a=
...
this.persistChange=function(a,b){var d=c.app.localStorage().collection(a.request.kidoService.service+"."+a.request.kidoService.collection),f=a._id,r="object"===typeof b?b._id:null;return r?d.get(f,!0).then(function(a){a._id=r;return d.persist(a)},function(){return c.getPendingRequestObject(f).then(function(a){a._id=r;return d.persist(a)})}).then(function(){return f!==r?d.del(f).then(function(){return e.persist({_id:f,new_id:r})}):$.Deferred().resolve()},function(){return $.Deferred().resolve()}):
d.del(f).then(function(){return e.del(f)})};this.getNewIdFromDictionary=function(a){return e.get(a,!0).then(function(a){return a.new_id})};this.ajax=function(a){if(!a)throw"The 'settings' argument is required.";var b=a.data,d=a.kidoService.service,e=a.kidoService.collection,f=a.kidoService.objectId,h=a.kidoService.query,l=a.kidoService.caching,m=a.kidoService.queueing,n=a.type.toLowerCase(),t="get"===n&&f,w="get"===n&&!f,B="post"===n&&b,u="put"===n&&b,v="delete"===n&&f,x="delete"===n&&!f,n=$.ajax(a),
s=$.Deferred();if(l){var p=c.app.localStorage().collection(d+"."+e),q=b?JSON.parse(b):null,A=q?q._id:null,d=f&&0>parseInt(f)||A&&0>parseInt(A);if(t||w)a.timeout=5E3;if(d){if(t)return p.get(f);if(v)return p.del(f).then(function(){return c.removePendingRequest(f)});u&&(a.type="POST",delete q._id,a.data=JSON.stringify(q),f=null)}t||w?n.then(function(a){return p.persist(a)}):B||u?n.then(function(a){q._id=a._id?a._id:q._id?q._id:null;q._metadata=a._metadata?a._metadata:q._metadata?q._metadata:null;return p.persist(q)}):
v?n.then(function(){return p.del(f?f:A)}):x&&n.then(function(){return p.drop()})}n.fail(function(d){if(0!==d.status)return s.reject(d);c.startCheckingConnectivity();if(m&&!l)if(t||w)s.reject(d);else{var e=(-1*$.now()).toString();c.addPendingRequest(e,a).then(function(){s.reject(d)})}else l?(e=function(b){if(m&&(B||u||v||x)){var d=x?(-1*$.now()).toString():v?f:u?A:b._id;c.addPendingRequest(d,a).then(function(){s.resolve(b)})}else s.resolve(b)},w?p.query(h).then(e):t?p.get(f).then(e):B||u?p.persist(b).then(e):
v?p.del(f).then(e):x&&p.drop().then(e)):s.reject(d)});n.done(function(a){s.resolve(a)});return s.promise()}};Kido.prototype.offline=function(){this._offline||(this._offline=new KidoOffline(this));return this._offline};
