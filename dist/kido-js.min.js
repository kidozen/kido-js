...
if(!JSON||!JSON.parse||!JSON.stringify)throw"KidoZen requires JSON.stringify. Try adding a polyfil lib like json2.js";if("undefined"===typeof $)throw"jQuery 1.8 or above is required to use the Kido SDK.";if("undefined"===typeof $.fn||"undefined"===typeof $.fn.jquery)throw"Could not determine jQuery version.";var $version=$.fn.jquery.split(".");if(2>parseInt($version[0])&&8>parseInt($version[1]))throw"jQuery 1.8 or above is required to use the Kido SDK.";
...
var b=$.Deferred(),e=window.open(a.signInUrl,"_blank","location=yes");h.isNative?(e.addEventListener("loadstop",function(){e.executeScript({code:"document.title;"},function(a){a=a[0];if(-1!==a.indexOf("Success payload=")){try{var c=m(a);h.authenticated=!0;b.resolve(c)}catch(l){b.reject("Unable to retrieve KidoZen token.")}e.close()}})}),e.addEventListener("loaderror",function(){b.reject("InAppBrowser error loading page.");e.close()}),e.addEventListener("exit",function(){b.reject("InAppBrowser has exited.")})):
window.addEventListener("message",function(c){var l=document.createElement("a");l.href=a.signInUrl;if(c.origin!==l.protocol+"//"+l.host)return b.reject("Unable to retrieve KidoZen token.");try{var d=m(c.data);h.authenticated=!0;b.resolve(d)}catch(f){b.reject("Unable to retrieve KidoZen token.")}e.close()},!1);return b.promise()}function g(a,b,c,d,f){var g;if("wrapv0.9"===f.protocol.toLowerCase())g=l;else if("ws-trust"===f.protocol.toLowerCase())g=e;else return $.Deferred().reject("Protocol not supported yet.");
return g({user:b,pass:c,scope:a.authServiceScope,endpoint:f.endpoint}).then(function(e){return(e=(/<Assertion(.*)<\/Assertion>/.exec(e)||[])[0])?$.ajax({url:a.authServiceEndpoint,type:"POST",data:{wrap_assertion:e,wrap_scope:a.applicationScope,wrap_assertion_format:"SAML"}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return $.Deferred().reject("Unable to retrieve KidoZen token.");h.authenticated=!0;w=b;x=c;y=d;return k(a)}):$.Deferred().reject("Unable to get a token from IDP")})}function m(a){if(-1===
a.indexOf("Success payload="))throw"Unable to retrieve KidoZen token.";a=JSON.parse(window.atob(a.replace("Success payload=","")));if(!a||!a.access_token&&!a.rawToken)throw"Unable to retrieve KidoZen token.";return k(a)}function k(a){var b=a.access_token?a.access_token:a.rawToken;a.token='WRAP access_token="'+b+'"';b=decodeURIComponent(b).split("&");a.claims=b;for(var e in b)if(b[e]){var c=b[e];if(-1<c.indexOf("ExpiresOn")){a.expiresOn=1E3*~~c.split("=")[1]-2E4;break}}return a}if(!(this instanceof
Kido))return new Kido;if("undefined"!==typeof c){if(-1===c.indexOf("://"))c="https://"+c;else if(0!==c.indexOf("http://")&&0!==c.indexOf("https://"))throw"Marketplace url must begin with https://";"/"===c.substr(-1)&&(c=c.substr(0,c.length-1))}var h=this,w=null,x=null,y=null,C=null;this.name=d||"local";this.marketplace=c;this.local="local"===this.name;this.authenticated=(this.hosted=!c)?!0:!1;this.isNative=-1===document.URL.indexOf("http://")&&-1===document.URL.indexOf("https://");"object"===typeof a&&
("object"===typeof a.token&&(this.inheritedToken=a.token),"string"===typeof a.username&&(w=a.username),"string"===typeof a.password&&(x=a.password),"string"===typeof a.provider&&(y=a.provider),"string"===typeof a.secretKey&&(C=a.secretKey));this.hosted||(this.getConfig=$.ajax({url:this.marketplace+"/publicapi/apps?name="+this.name}).then(function(a){if(!a||!a.length)return $.Deferred().reject("Application configuration not found.");h.url=a[0].url.substring(0,a[0].url.length-1);return a[0]},function(){return $.Deferred().reject("Unable to retrieve application configuration. Either there is no internet connection or the marketplace url is invalid.")}));
this.authenticate=function(){if(h.hosted)return $.Deferred().resolve("No need to authenticate to this Web App");var a=arguments;h.token=h.getConfig.then(function(b){if(0===a.length&&h.inheritedToken)return h.authenticated=!0,k(h.inheritedToken);if(0===a.length)return f(b.authConfig);var e=b.authConfig.identityProviders,c=a[2]||Object.keys(e)[0];return(e=e[c])?e.protocol?g(b.authConfig,a[0],a[1],a[2],e):$.Deferred().reject("Invalid Identity Provider Protocol"):$.Deferred().reject("Invalid Identity Provider")});
return h.token};this.send=function(a){if(!a.url)return $.Deferred().reject({msg:"Not a valid ajax URL.",error:"Invalid URL"});var e=$.extend({},{type:"POST",cache:!0,headers:{"X-Kido-SDK":"JS","X-Kido-SDK-Version":KIDO_SDK_VERSION||"UNKNOWN"}},a);e.data&&!e.contentType&&(e.contentType="application/json");return!h.hosted&&h.authenticated?h.token.then(function(a){if(a.expiresOn<(new Date).getTime()){if(w&&x&&y)return h.authenticate(w,x,y);if(!C)return h.authenticate();var b=a.refresh_token;h.token=
h.getConfig.then(function(a){return $.ajax({url:a.authConfig.oauthTokenEndpoint,type:"POST",data:{grant_type:"refresh_token",client_id:a.domain,client_secret:C,scope:a.authConfig.applicationScope,refresh_token:b}}).then(function(a){if(!a||!a.access_token&&!a.rawToken)return h.authenticated=!1,$.Deferred().reject("Unable to retrieve KidoZen token.");a.refresh_token=b;return k(a)})});return h.token}return a}).then(function(a){e.url=h.url+e.url;e.headers=e.headers||{};e.headers.authorization=a.token;
return b(e)}):b(e)};this.get=function(a,b){b=$.extend({url:a,type:"GET"},b||{});return h.send(b)};this.post=function(a,b,e){e=$.extend({url:a,type:"POST",data:b?JSON.stringify(b):null},e||{});return h.send(e)};this.del=function(a,b){b=$.extend({url:a,type:"DELETE"},b||{});return h.send(b)}};var KidoConfig=function(d){if(!(this instanceof KidoConfig))return new KidoConfig(d);if(!d)throw"The 'kidoApp' argument is required by the KidoConfig class.";var c=this;this.app=d;this.SERVICE_NAME="config";this.set=function(a,b){if(!a)throw"The config key 'name' is required to set a value.";return c.app.post("/config/"+a,b)};this.get=function(a){if(!a)throw"The config key 'name' is required to retrieve the value.";return c.app.get("/config/"+a)};this.getAll=function(){return c.app.get("/config")};
...
d.del(f).then(function(){return e.del(f)})};this.getNewIdFromDictionary=function(a){return e.get(a,!0).then(function(a){return a.new_id})};this.ajax=function(a){if(!a)throw"The 'settings' argument is required.";var b=a.data,d=a.kidoService.service,e=a.kidoService.collection,f=a.kidoService.objectId,g=a.kidoService.query,l=a.kidoService.caching,m=a.kidoService.queueing,n=a.type.toLowerCase(),s="get"===n&&f,v="get"===n&&!f,B="post"===n&&b,t="put"===n&&b,u="delete"===n&&f,z="delete"===n&&!f,n=$.ajax(a),
